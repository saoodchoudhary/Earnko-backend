...
function headerVariants() {
  ...
  return [
    { ...base, token: API_KEY },
    { ...base, Authorization: `Token token=${API_KEY}` },
...
async function buildDeeplink({ url, subid, channel_id, subid2, subid3, subid4, subid5 }) {
  ...
  const short = json?.short_url || json?.shortened_url || json?.link?.short_url || json?.link?.shortened_url || json?.data?.short_url || json?.data?.shortened_url || null;
  const affiliate = json?.affiliate_url || json?.link?.affiliate_url || json?.data?.affiliate_url || null;
  if (short) return short;
  if (affiliate) return affiliate;
  const err = new Error('Cuelinks: no short_url in response'); err.body = json; throw err;
}
...
async function getCampaigns({ search_term = '', page = 1, per_page = 30, country_id, categories } = {}) {
  ...
  params.set('page', String(page)); params.set('per_page', String(per_page));
  const cid = country_id || CUELINKS_COUNTRY_ID; if (cid) params.set('country_id', String(cid));
